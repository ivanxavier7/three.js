...
const generateGalaxy = () => {
    ...
    for(let i = 0; i < parameters.count; i++) {
        ...
        const branchAndgle = (i % parameters.branches) / parameters.branches * Math.PI * 2
        ...
        positions[i3    ] = Math.cos(branchAndgle + spinAngle) * radius + randomX
        positions[i3 + 1] = randomY
        positions[i3 + 2] = Math.sin(branchAndgle + spinAngle) * radius + randomZ

        // Gradient color
        const gradientColor = colorInside.clone()
        gradientColor.lerp(colorOutside, radius / parameters.radius)
...
gui.add(parameters, 'randomnessPower').min(1).max(19).step(0.001).onFinishChange(generateGalaxy)
