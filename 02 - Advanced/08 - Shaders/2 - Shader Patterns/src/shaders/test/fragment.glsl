...
void main()
{
    ...
    // Pattern 30 - Center gradient but small white like a sun
    //float strength = 0.015 / distance(vUv, vec2(0.5, 0.5));

    // Pattern 31 - Same as 30 but stretch, elipse sun
    //vec2 lightUv = vec2(
    //    vUv.x * 0.1 + 0.45,
    //    vUv.y * 0.5 + 0.25
    //);
    ...
    // Pattern 33 - Rotated Star using function
    ...
    // Pattern 34 - Square empty circle in the middle
    //float strength = step(0.25, distance(vUv, vec2(0.5)));

    // Pattern 35 - Eclipse with gradient inside and outside
    //float strength = abs(distance(vUv, vec2(0.5)) - 0.25);

    // Pattern 36 - Empty Circle
    //float strength = step(0.01, abs(distance(vUv, vec2(0.5)) - 0.25));

    // Pattern 37 - Circle
    //float strength = 1.0 - step(0.01, abs(distance(vUv, vec2(0.5)) - 0.25));

    // Pattern 38 - Elastic (Circle with waves)
    //vec2 waveUv = vec2(
    //    vUv.x,
    ...
    // Pattern 39 - Elastic with X (makes some particles)
    //vec2 waveUv = vec2(
    ...
    // Pattern 41 - Complex, like a CPU connector
    //vec2 waveUv = vec2(
    //    vUv.x + sin(vUv.y * 300.0) * 0.1,
    //    vUv.y + sin(vUv.x * 300.0) * 0.1     // 35 Frequency
    //);
    ...
    // Pattern 42 - 45 % angle gradient
    //float angle = atan(vUv.x, vUv.y);
    //float strength = angle;

    // Pattern 43 - 45% angle gradient and screen cuted in the middle
    //float angle = atan(vUv.x - 0.5, vUv.y - 0.5);
    //float strength = angle;

    // Pattern 44 - 45% gradient like a clock
    //float angle = atan(vUv.x - 0.5, vUv.y - 0.5);
    //angle /= PI * 2.0;
    //angle += 0.5;
    //float strength = angle;
    ...
    // Pattern 46 - Illusion with 20 triangles and bigger dark space
    //float angle = atan(vUv.x - 0.5, vUv.y - 0.5);
    //angle /= PI * 2.0;
    //angle += 0.5;
    //float strength = sin(angle * 100.0);

    // Pattern 47 - Waving circle
    //float angle = atan(vUv.x - 0.5, vUv.y - 0.5);
    //angle /= PI * 2.0;
    //angle += 0.5;
    ...
    // Pattern 48 - Perlin Noise - Clouds, Water, Fire, Terrain, Elevation and to animate Grass and Snow (Nature Algorithm)
    // Noise templates - https://gist.github.com/patriciogonzalezvivo/670c22f3966e662d2f83
    //float strength = cnoise(vUv * 20.0);

    // Pattern 49 - Camouflage
    //float strength = step(0.1, cnoise(vUv * 10.0));

    // Pattern 50 - Neon Camouflage
